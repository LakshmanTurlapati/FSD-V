...
public class GTA5StateStreamer : Script
{
    ...
    {
        InitLog();
        Log("INFO", "=== GTA5StateStreamer loaded ===");
        Log("INFO", "State pipe: " + PIPE_NAME + " | Control pipe: " + CONTROL_PIPE_NAME);
        Tick += OnTick;
        Aborted += OnAborted;
        StartPipeServer();
        StartControlPipeServer();
        Log("INFO", "Pipe servers started — waiting for Python client to connect");
    }
    ...
    private void StartPipeServer()
    {
        ...
        {
            Log("INFO", "State pipe thread started — creating pipe " + PIPE_NAME);
            while (true)
            {
                try
                {
                    ...
                    Log("INFO", "State pipe created — waiting for client connection...");
                ...
                {
                    Log("WARN", "State pipe error: " + ex.GetType().Name + " " + ex.Message);
    ...
    private void OnTick(object sender, EventArgs e)
    {
        if (!_firstTickLogged)
        {
            ...
            Log("INFO", "First OnTick fired — script is alive");
        }
    ...
    private void ApplyDriveCommand()
    {
        ...
        Function.Call((Hash)0xE8A25867FBA3B05E, CONTROL_GROUP, INPUT_VEH_MOVE_LR, cmd.steer);
        Function.Call((Hash)0xE8A25867FBA3B05E, CONTROL_GROUP, INPUT_VEH_ACCELERATE, cmd.throttle);
        Function.Call((Hash)0xE8A25867FBA3B05E, CONTROL_GROUP, INPUT_VEH_BRAKE, cmd.brake);
        Function.Call((Hash)0xE8A25867FBA3B05E, CONTROL_GROUP, INPUT_VEH_HANDBRAKE, cmd.handbrake ? 1f : 0f);
    }
    ...
    private string BuildJson(long now, int playerId, int ped, int veh)
    {
        ...
        try
        {
            ...
            Vector3 rn = op.GetResult<Vector3>();
            rdHead = oh.GetResult<float>();
            rdDist = Dist2D(pos, rn);
        }
        catch { }
        ...
        // Memory-only fields (no native equivalent — 0 until SHVDN updates)
